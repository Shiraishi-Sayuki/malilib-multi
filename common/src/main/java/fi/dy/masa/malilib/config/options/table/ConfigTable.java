...
public class ConfigTable extends ConfigBase<ConfigTable> implements IConfigTable
{
	public static final Codec<ConfigTable> CODEC = RecordCodecBuilder.create(
			inst -> inst.group(
					...
					CodecsWrap.listOrSingle(PrimitiveCodec.STRING.listOf()).fieldOf("defaultTable").forGetter(get ->
					                                                                                      {
					...
					CodecsWrap.listOrSingle(PrimitiveCodec.STRING.listOf()).fieldOf("table").forGetter(get ->
					                                                                               {
						                                                                               List<List<String>> table = new ArrayList<>();
					...
					CodecsWrap.listOrSingle(PrimitiveCodec.STRING.listOf()).fieldOf("labels").forGetter(get ->
                                                                                                   {
		...
		public ConfigTable build(boolean ignoreWarning)
		{
			...
			for (TableRow v : this.defaultValue)
			{
				for (int j = 0; j < this.types.length; j++)
				{
                    ...
                    if (this.allowAddNewEntry && this.types[j] == EntryTypes.LABEL && Services.PLATFORM.isDevelopmentEnvironment() && !ignoreWarning)
                    {
